...
{
    public class AzureAcquaintanceSource : IDataSource
    {
		...
		protected virtual void RaiseDataSyncErrorEvent(DataSyncErrorEventArgs e)
		{
			DataSyncErrorEventHandler handler = OnDataSyncError;
        ...
        public async Task<IEnumerable<Notification>> GetNotificationItems()
        {
            return await Execute<IEnumerable<Notification>>(async () =>
            {
                await SyncItemsAsync().ConfigureAwait(false);
                return await _notificationTable.Where(x => x.DataPartitionId == _DataPartitionId).ToEnumerableAsync().ConfigureAwait(false);
            }, new List<Notification>()).ConfigureAwait(false);
        }
        ...
        async Task<bool> Initialize()
        {
            return await Execute<bool>(async () =>
            {
				...
				_SyncHandler.OnDataSyncError += (object sender, DataSyncErrorEventArgs e) => {
