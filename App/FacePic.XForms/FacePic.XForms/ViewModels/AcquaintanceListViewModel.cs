...
namespace Acquaint.XForms
{
    public class AcquaintanceListViewModel : BaseNavigationViewModel
    {
        ...
        ObservableRangeCollection<Client> _Acquaintances;
		...
		void SetDataSource()
		{
			_DataSource = ServiceLocator.Current.GetInstance<IDataSource>();
		}

        public ObservableRangeCollection<Client> Acquaintances
        {
            get { return _Acquaintances ?? (_Acquaintances = new ObservableRangeCollection<Client>()); }
            set
            {
        ...
        async Task FetchAcquaintances()
        {
            ...
            Acquaintances = new ObservableRangeCollection<Client>(await _DataSource.GetItems());
        ...
        {
            MessagingService.Current.Subscribe<Client>(MessageKeys.AddAcquaintance, async (service, acquaintance) =>
            {
                IsBusy = true;
        ...
        {
            MessagingService.Current.Subscribe<Client>(MessageKeys.UpdateAcquaintance, async (service, acquaintance) =>
            {
                IsBusy = true;
        ...
        {
            MessagingService.Current.Subscribe<Client>(MessageKeys.DeleteAcquaintance, async (service, acquaintance) =>
            {
                IsBusy = true;
